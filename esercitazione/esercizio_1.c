...
ogni campo deve avere la dimensione  in ettare (int),
il tipo di frutta/verdura (char[12])
la regione di appartenenza (char[2])
il prezzo di vendita del campo (long int)
la data di raccolta del raccolto (char[12]) formato mm/aa

creare la struttura dati necessaria chiamata lista_campi

predisporre una funzione per aggiungere i campi 
predisporre una funzione per vedere la regione e la dimensione passando il tipo di frutta/verdura che si cerca
